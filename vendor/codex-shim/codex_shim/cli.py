...
def _doctor_python() -> list[DoctorCheck]:
    ...
    # This vendored subset is compiled and exercised on the Ubuntu WSL Python
    # shipped with this project environment (3.10+).
    status = "OK" if sys.version_info >= (3, 10) else "FAIL"
    detail = "" if status == "OK" else "this integration requires Python 3.10+"
    return [
...
def install_codex_config(settings_path: Path, port: int, model_slug: str | None = None) -> None:
    ...
    # Migrate the old single-provider PCSS setup. Keeping this inactive block
    # still makes Codex request PCSS_VLLM_API_KEY during provider discovery.
    cleaned = _remove_section(cleaned, "model_providers.pcss-vllm")
    ...
    if "pcss-vllm" in previous_top_level.get("model_provider", ""):
        previous_top_level = {}
